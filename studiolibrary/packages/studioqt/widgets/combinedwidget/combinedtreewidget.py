...
class CombinedTreeWidget(CombinedItemViewMixin, QtWidgets.QTreeWidget):
    ...
    def _items(self):
        ...
        :rtype: lsit[studioqt.TreeWidgetItem]
        """
        return self.findItems('*', QtCore.Qt.MatchWildcard | QtCore.Qt.MatchRecursive)
    ...
    def textFromColumn(self, column, split = None, duplicates = False):
        ...
        """
        items = self.items()
        results = self.textFromItems(items, column, split=split, duplicates=duplicates)
        return results
    ...
    def columnSettings(self):
        ...
        for column in range(self.columnCount()):
            ...
            width = self.columnWidth(column)
            columnSettings[label] = {'hidden': hidden,
             'width': width,
             'index': column}
